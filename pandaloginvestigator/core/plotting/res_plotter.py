...
def plot_instruction_results(dict_list, clean_dicts):
    do_stuff(dict_list[0], 'b', 'H', 'Total', 100)
    do_stuff(dict_list[1], 'g', 'o', 'Malware from database', 100)
    do_stuff(dict_list[2], 'r', 'o', 'Created processes', 10)
    do_stuff(dict_list[3], 'y', 'o', 'Memory written processes', 10)

    # Clean dictionaries
    do_stuff(clean_dicts[0], 'm', 'o', 'Total without crashes/errors', 100)
    do_stuff(clean_dicts[1], 'g', 'o', 'Malware from database without crashes/errors', 100)
    do_stuff(clean_dicts[2], 'r', 'o', 'Created processes without crashes/errors', 10)
    do_stuff(clean_dicts[3], 'y', 'o', 'Memory written processes without crashes/errors', 10)


# Auxiliary method for generating statistics, pruning values and plotting.
def do_stuff(chosen_dict, color, shape, title, prune):
    stats = utils.compute_stats(chosen_dict)
    plot_data(chosen_dict, stats, color, shape, title)
    chosen_dict_pruned = utils.prune_data(chosen_dict, prune)
    stats = utils.compute_stats(chosen_dict_pruned)
    plot_data(chosen_dict_pruned, stats, color, shape, title + ' pruned')
